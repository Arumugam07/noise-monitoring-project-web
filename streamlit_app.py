...
OFFLINE_THRESHOLD = 0.30  # < 30% data = offline
DEGRADED_THRESHOLD = 0.70  # < 70% data = degraded
...
def show_login_page():
    ...
    st.markdown("""
        ...
        .login-card { background: white; padding: 2rem; border-radius: 15px;
            box-shadow: 0 10px 30px rgba(0,0,0,0.1); border: 1px solid #e9ecef; }
        ...
        .divider { height: 1px; background: linear-gradient(to right, transparent, #dee2e6, transparent);
            margin: 1.5rem 0; }
        </style>
        ...
        st.markdown("""
            <div class="feature-list">
                ...
                <div class="feature-item"><span class="feature-icon">🔴</span>
                    <span>Real-time monitoring across 13 locations</span></div>
                <div class="feature-item"><span class="feature-icon">📊</span>
                    <span>Advanced filtering and data analysis</span></div>
                <div class="feature-item"><span class="feature-icon">📥</span>
                    <span>Export data in CSV and Excel formats</span></div>
                <div class="feature-item"><span class="feature-icon">⚠️</span>
                    <span>Automatic offline station detection</span></div>
                <div class="feature-item"><span class="feature-icon">🔍</span>
                    <span>Historical data search and filtering</span></div>
            </div>
        """, unsafe_allow_html=True)
...
def main():
    ...
    st.markdown("""
        ...
        .latest-reading-card { padding: 1rem; border-radius: 8px; margin-bottom: 1rem;
            border-left: 5px solid; background-color: #f8f9fa; transition: transform 0.2s; }
    ...
    with st.sidebar.expander("ℹ️ About", expanded=False):
        ...
        **Noise Monitoring System**
        ...
        Displays noise level readings (dB) collected every minute from monitoring stations across Singapore.
    ...
    # FIX: Default to yesterday → 6 days prior (7 complete days, no partial today)
    # Avoids Supabase statement timeout caused by loading all-time data on first load
    today = date.today()
    ...
    date_selection = st.sidebar.date_input(
        ...
        help="Defaults to last 7 complete days. Today excluded to avoid partial data."
    )
    ...
    vmin = None
    if use_min:
        vmin = st.sidebar.number_input("Minimum Value (dB)", value=40.0)
        value_filter_active = True

    use_max = st.sidebar.checkbox("Filter by maximum value", value=False)
    vmax = None
    if use_max:
        vmax = st.sidebar.number_input("Maximum Value (dB)", value=100.0)
        value_filter_active = True

    if value_filter_active:
        st.sidebar.info("🔍 Value filters search ALL data in date range (may be slower)")

    st.sidebar.markdown("---")
    st.sidebar.subheader("🔊 Persisted Noise Detection")

    detect_persisted = st.sidebar.checkbox("Enable Detection", value=False,
                                           help="Detect sustained high noise periods")
    ...
    if detect_persisted:
        ...
        persist_duration = st.sidebar.number_input(
            "Duration (minutes)", min_value=1, max_value=60, value=3, step=1,
            help="Minimum consecutive minutes to flag as incident"
        )
        st.sidebar.info(f"🔍 Detecting {persist_min_db}-{persist_max_db}dB for {persist_duration}+ minutes")
    ...
    if not value_filter_active and not is_single_day_view:
        page = st.sidebar.number_input(
            "Page Number", min_value=0, value=0, step=1,
            help=f"Each page shows {PAGE_SIZE} rows",
        )
    else:
        page = 0
    ...
    st.markdown('<div class="sub-header">Real-time noise level monitoring across multiple locations in Singapore</div>',
                unsafe_allow_html=True)
    ...
    try:
        ...
        with st.spinner("Loading data from database..."):
            ...
            if value_filter_active:
                st.info("🔍 Searching all records for values matching your criteria...")
        ...
        if not filtered.empty:
            ...
            if value_filter_active:
                ...
                location_counts = {loc: filtered[loc].notna().sum() for loc in location_cols if loc in filtered.columns}
                ...
                for i in range(0, len(sorted_locations), 3):
                    ...
                    for j, col_obj in enumerate(cols):
                        if i + j < len(sorted_locations):
                            ...
                            if count == 0:
                                color, intensity = "#6c757d", "None"
                            elif count < 10:
                                color, intensity = "#28a745", "Low"
                            elif count < 50:
                                color, intensity = "#ffc107", "Medium"
                            elif count < 100:
                                color, intensity = "#fd7e14", "High"
                            else:
                                color, intensity = "#dc3545", "Very High"

                            with col_obj:
                                st.markdown(
                                    f"""<div class="latest-reading-card" style="border-left-color: {color};">
                                        ...
                                        <div style="font-size: 2.5rem; font-weight: bold; color: {color}; margin: 0.5rem 0;">
                                            {count} <span style="font-size: 1.2rem;">times</span></div>
                ...
                if is_single_date:
                    ...
                    st.info(
                        ...
                        f"✅ {online_count} Online | ⚠️ {degraded_count} Degraded | ❌ {offline_count} Offline"
                    )
                    ...
                    for i in range(0, len(sorted_sensors), 3):
                        ...
                        for j in range(3):
                            if i + j < len(sorted_sensors):
                                ...
                                messages = {'ONLINE': 'Fully operational', 'DEGRADED': 'Monitor closely',
                                            'OFFLINE': 'Needs maintenance'}

                                with cols[j]:
                                    st.markdown(
                                        f"""<div style="background-color: {color['bg']}; border-left: 5px solid {color['border']};
                                            ...
                                            <div style="font-size: 1.5rem; font-weight: bold; color: {color['text']};">
                                                {icons[h['status']]} {h['status']}</div>
                                            <div style="font-size: 1.1rem; font-weight: 600; color: #333;">
                                                {h['reading_count']:,}/{READINGS_PER_DAY:,}</div>
                    ...
                    if detect_persisted:
                        ...
                        with st.spinner("Analyzing noise patterns..."):
                            incidents = detect_persisted_noise_incidents(
                                filtered, location_cols, persist_min_db, persist_max_db, persist_duration
                            )
                        ...
                        if incidents:
                            ...
                            st.info(f"✓ No persisted noise incidents detected")
                    ...
                    st.caption("📊 Status: ✅ Online (≥70%) | ⚠️ Degraded (40-70%) | ❌ Offline (<40%)")
                    ...
                    st.info(
                        ...
                        f"✅ {online_count} Operational | ⚠️ {degraded_count} Degraded | ❌ {offline_count} Critical"
                    )
                    ...
                    if detect_persisted and 'incidents' in locals():
                        for incident in incidents:
                            ...
                            incidents_by_location[loc] = incidents_by_location.get(loc, 0) + 1

                    status_order = {'OFFLINE': 0, 'DEGRADED': 1, 'ONLINE': 2}
                    sorted_sensors = sorted(health.items(),
                                            key=lambda x: (status_order[x[1]['status']], -len(x[1]['offline_dates'])))
                        ...
                        for j in range(3):
                            if i + j < len(sorted_sensors):
                                ...
                                card_html = (
                                    ...
                                    f'border-radius: 8px; padding: 1rem; margin-bottom: 0.5rem; height: 220px; '
                                    f'display: flex; flex-direction: column; justify-content: space-between;">'
                                    f'<div style="font-size: 0.85rem; font-weight: 600; color: #333;">📍 {loc}</div>'
                                    f'<div style="font-size: 1.3rem; font-weight: bold; color: {text_color};">'
                                    f'{icon} {h["status"]} ({int(h["completeness_pct"])}%)</div>'
                                    f'<div style="font-size: 0.9rem; color: #333;"><strong>Days online:</strong> '
                                    f'{h["online_days"]}/{h["total_days"]}</div>'
                                    f'<div style="font-size: 0.85rem; color: #666;"><strong>Readings:</strong> '
                                    f'{h["total_readings"]:,}/{h["expected_readings"]:,}</div>'
                                )

                                if detect_persisted and incident_count > 0:
                                    card_html += (
                                        f'<div style="font-size: 0.85rem; color: #d63384;">⚠️ Persisted noise: '
                                        f'{incident_count} incidents</div>'
                                    )

                                card_html += (
                                    f'<div style="font-size: 0.75rem; color: {text_color};">{issues_text}</div>'
                                    f'<div style="font-size: 0.8rem; font-weight: 600; color: {text_color};">{severity}</div>'
                                    f'</div>'
                                )
            ...
            if numeric_cols:
                ...
                if all_values:
                    with col2:
                        st.metric("Average Reading", f"{sum(all_values)/len(all_values):.1f} dB")
                    with col3:
                        st.metric("Min Reading", f"{min(all_values):.1f} dB")
                    with col4:
                        st.metric("Max Reading", f"{max(all_values):.1f} dB")
            ...
            if value_filter_active:
                table_data = filtered
                st.caption(f"Showing **all {len(table_data)}** records matching your filter criteria.")
            else:
                table_data = filtered_table
                st.caption(f"Showing **{len(table_data)}** rows from page **{page + 1}** (page size: {PAGE_SIZE}).")
            ...
            - **Refresh MV**: Run `REFRESH MATERIALIZED VIEW public.wide_view_mv;` in Supabase SQL Editor
            """)
        ...
        with st.expander("🔧 Setup Instructions", expanded=True):
            ...
            **The database might not be configured yet, or credentials are missing.**
            ...
            1. **Create the materialized view** in Supabase SQL Editor:
            ...
            CREATE UNIQUE INDEX idx_wide_view_mv_unique ON public.wide_view_mv ("Date", "Time");
            REFRESH MATERIALIZED VIEW public.wide_view_mv;
            ```

            2. **Set up auto-refresh** (run once in SQL Editor):
            ```sql
            CREATE EXTENSION IF NOT EXISTS pg_cron;
            SELECT cron.schedule('refresh-wide-view-mv', '0 17 * * *',
              'REFRESH MATERIALIZED VIEW CONCURRENTLY public.wide_view_mv');
            ```

            3. **Set environment variables** or Streamlit secrets:
               - `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `APP_USERNAME`, `APP_PASSWORD`
            """)
